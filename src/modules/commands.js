...
module.exports = (plugin, config) => {
  ...
  plugin.cmdListeners.set('404', async (msg, cmd) => {
    plugin.foxLog(plugin.localeString('fox.0001', pl.userWithDescriminator(msg.author).xb16, msg.guild.name, cmd.xb16) + '[FOX#0001]'.x240)
  ...
  plugin.cmdListeners.set('off', async (msg, cmd) => {
    plugin.foxLog(plugin.localeString('fox.0002', pl.userWithDescriminator(msg.author).xb16, msg.guild.name, cmd.xb16) + '[FOX#0002]'.x240)
  ...
  plugin.cmdListeners.set('use', async (msg, cmd, args) => {
    plugin.foxLog(plugin.localeString('fox.use', pl.userWithDescriminator(msg.author).xb16, msg.guild.name, cmd.xb16 + (args.length > 0 ? ' '.xb16 + args.join(' ').xb16 : '')))
  })
